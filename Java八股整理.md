...
还有一种表锁叫元数据锁（metadata lock MDL），增删改查时自动加上MDL读锁，对表结构修改自动加上MDL写锁，读写，写写阻塞。

![image-20240806140945642](Java八股整理.assets/image-20240806140945642.png)
...
- **优点：**
  ...
  - **空间整合**：整体上看是~~“标记-整合”~~算法，从局部的两个Region之间看是==“复制算法”==，意味着不会产生内存碎片；
    ...
    ![image-20240704212909872](Java八股整理.assets/image-20240704212909872.png)
...
- **执行构造方法：**字节码`<init>`给对象里面的属性赋值（初始化零值的那些值）；构造器里面写的代码逻辑。（类加载的cinit方法可以认为是静态代码块）
...
![image-20240704215939141](Java八股整理.assets/image-20240704215939141.png)
...
分为手机端，PC端，服务端。二维码可以通过解码解析成一段二维码ID。二维码的状态有`未扫描、已失效、已成功`，存储在服务端，由PC端请求并展示在网页上。

1. 访问PC端二维码生成页面，PC端请求服务端获取`二维码ID`
2. 服务端生成相应的`二维码ID`，设置二维码的过期时间，状态等
3. PC端通过服务端返回的`二维码ID`生成一个`二维码`
4. 手机端扫描`二维码`并解析成`二维码ID`
5. 手机端通过`手机端token、设备信息、二维码ID`，请求服务端
6. 服务端校验`手机端token`，根据`手机端token`和`二维码ID`生成`PC端token`
7. PC端通过`轮询/长轮询/WebSocket`方式请求服务端，通过`二维码ID`获取二维码状态，如果已成功，返回`PC token`，登录成功(IO多路复用，用ReentrantLock，c1.await()，c1.signal()？)
