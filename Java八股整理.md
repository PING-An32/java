...
##### 1.4 Parallel Scavenge 收集器（复制算法）
...
- 是一种并发垃圾收集器。采用标记-清除算法。具有高并发，**追求最短停顿时间**的特点。收集过程分为以下四个阶段：
  ...
  + **初始标记：**暂停其他线程，对GC ROOT直接相关联的对象进行标记，很快；
...
- G1的垃圾收集范围不再是Minor GC，Major或者Full GC了，它可以面向堆内存任何部分来组成回收集，哪块内存存放的垃圾越多，回收的收益越大，这就是G1收集器的**Mixed GC模式**。
...
- **类加载检查：**虚拟机遇到一条new指令时，首先将去检查这个指令的参数能否在常量池中**定位到这个类的符号引用**，并且检查这个符号引用代表的类是否已经被加载、解析和初始化过，如果没有，那必须先执行相应的类加载过程。
...
类从被加载到虚拟机内存中开始，到卸载出内存为止，它的整个生命周期包括：加载、验证、准备、解析、初始化、使用、卸载7个阶段。其中验证、准备、解析3个部分统称为连接，这7个阶段发生的顺序如下图所示： 
...
1. **加载**   ✅
   【简洁】：将类文件字节流加载到内存中。其中类信息到方法区中，也会生成一个`java.lang.Class`对象在**方法区**中。

   在加载阶段，虚拟机需要完成以下三件事情：

   + 通过一个类的全限定名来获取定义此类的二进制字节流；
   + 将这个字节流所代表的的静态存储结构转化为方法区的运行时数据结构；
   + 在内存中生成一个代表这个类的java.lang.Class对象，作为方法区中这个类的各种数据的访问接口
